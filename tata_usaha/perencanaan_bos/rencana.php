...
foreach ($standar as $st) {
  ...
  if ($data['tahun'] > 2018) {
    $tahun1 = $data['tahun'] - 1;
    $tahun2 = $data['tahun'] - 2;
    $total_dana_rencana1 = $crud->query("SELECT SUM(jumlah) jumlah FROM bos_rkas JOIN bos_rkas_detail ON bos_rkas.id_bos_rkas = bos_rkas_detail.bos_rkas WHERE bos_rkas.tahun_ajaran = '" . $tahun2 . "'")[0]['jumlah'];
    $total_dana_rencana2 = $crud->query("SELECT SUM(jumlah) jumlah FROM bos_rkas JOIN bos_rkas_detail ON bos_rkas.id_bos_rkas = bos_rkas_detail.bos_rkas WHERE bos_rkas.tahun_ajaran = '" . $tahun1 . "'")[0]['jumlah'];
    ...
    $dana_rencana2 = $crud->query("SELECT SUM(jumlah) jumlah FROM bos_rkas JOIN bos_rkas_detail ON bos_rkas.id_bos_rkas = bos_rkas_detail.bos_rkas WHERE bos_rkas.npsn = '" . $st['idsnp'] . "' AND bos_rkas.tahun_ajaran = '" . $tahun1 . "'")[0]['jumlah'];
